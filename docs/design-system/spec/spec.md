...
| [UC-02 — A reviewer audits the system at one URL](./use-cases.md#uc-02--a-reviewer-audits-the-system-at-one-url-us-02) | A designer or reviewer runs `npm run storybook` and audits every token, type style, and primitive in one Storybook surface (port 6006) — see [deltas.md 2026-04-28-storybook](../deltas.md) for the migration off `/design-system` | US-02 |
...
| 2026-04-28 | Storybook supersedes `/design-system` route | User asked for a Storybook-based preview surface instead of an in-app route ("I don't want this running in an internal route of the main server"). REQ-008 superseded: preview surface is Storybook 10.3.5 with `@storybook/nextjs-vite`, port 6006 (`npm run storybook`), static export via `npm run build-storybook`. REQ-012 superseded: smoke contract moves from a single page-render assertion to (a) `*.stories.tsx` play functions running in headless Chromium via `@storybook/addon-vitest` and (b) a thin unit smoke at `src/__tests__/primitives-smoke.test.tsx` that imports each primitive directly. Files removed: `app/(internal)/design-system/page.tsx`, `app/(internal)/layout.tsx`, `src/__tests__/design-system-page.test.tsx`. Files added: 8 component stories, 4 token MDX docs (`Colors`, `Typography`, `Layout`, `Brand` under `.storybook/docs/`), `.storybook/{main.ts,preview.ts}`, `src/__tests__/primitives-smoke.test.tsx`. CI quality job adds `npm run build-storybook` after tests. See [deltas.md 2026-04-28-storybook](../deltas.md) for the full delta. |
