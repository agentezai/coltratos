...
- 3 fixture directories. Reuse `tests/fixtures/pliegos/` PDFs where possible (any 3 of the 5 from pdf-ingestion's corpus). For each fixture:
  ...
  - `ingestionResult.json` — pre-computed `IngestionResult` from running the pdf-ingestion inner pipeline on the pliego (snapshot, regenerable). Replaces the old `segments.json`.
  ...
  - `expected_requisitos.json` — manually curated by the founder (v1) listing the requisitos that an expert reviewer agreed with: `categoria` (narrow `RequisitoCategoria` — `juridico`/`financiero`/`tecnico`/`experiencia`, NEVER `general`), `descripcion`, `cumple` verdict, `pagina_fuente` (1-indexed page number), `quote_fuente` (verbatim), `citation_unverified` (boolean — `false` for well-cited fixtures), `is_habilitante` (boolean), `is_habilitante_source` (`'structural'` or `'llm'`; `'manual'` not produced in v1). Total ~10–25 requisitos per fixture.
...
- For each fixture:
  1. Load `pliego`, `ingestionResult`, `empresa`, `expectedRequisitos` from disk.
  2. Instantiate `AnthropicRequisitosExtractor({ client: new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY }), logger: testLogger })`.
  3. Call `extract({ pliego, ingestionResult, empresa, analisisId })`.
  ...
  10. **Partial-result assertion.** Assert `output.warning === undefined` on clean fixture runs. Any fixture that resolves with `warning !== undefined` is flagged as a quality signal in the test output (does not fail CI on its own, but is logged prominently).
  11. **Page citation range assertion.** For every emitted requisito, assert `pagina_fuente >= 1 && pagina_fuente <= ingestionResult.pages.length`.
...
- [ ] 3 fixture directories exist under `tests/fixtures/golden/extraction/` with `pliego.pdf` (or symlink), `ingestionResult.json`, `empresa.json`, `expected_requisitos.json`.
...
- [ ] All `expected_requisitos.json` fixtures carry `pagina_fuente` and `quote_fuente` (replacing `citation_segment_id` / `citation_quote`).
- [ ] Clean fixture runs produce `output.warning === undefined`; any `warning` is logged prominently as a quality signal.
