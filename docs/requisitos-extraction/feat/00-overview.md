...
- Key approach: dependency-injected SDK client + logger; `cache_control: ephemeral` blocks on system prompt + JSON schema and on empresa profile; one Claude call per `RequisitoCategoria` (max 4, all pages per call) via `Promise.all`; Zod-validated structured output with one internal retry per category; partial result with `warning` + `failed_categories` when one or more categories fail on second retry; verbatim citation verification (`pagina_fuente` + NFKC-normalized `quote_fuente` substring match) as a soft signal; hard cost ceiling at $0.05 USD per call enforced via `AbortController`.
- Output: `ExtractorOutput = { requisitos: Requisito[], costUsd: number, modelMetadata: ModelMetadata, warning?: string, failed_categories?: RequisitoCategoria[] }` consumed by the future `analisis-orchestration` spec, which owns persistence and the `(pliego.file_hash, empresa.id, implementation_id)` idempotency cache.
...
flowchart TD
    subgraph Caller["Caller (future analisis-orchestration spec)"]
        Input["ExtractorInput\n{ pliego, ingestionResult, empresa, analisisId }\n[assembled from pdf_pages per ADR-010]"]
    end
...
| T7 | [01-plan-07-eval-harness-gate.md](./01-plan-07-eval-harness-gate.md) | extraction-eval-harness CI gate + nybo-verify Verified check: ≥85% aggregate / ≥80% per-tipo recall required before Verified | T6 |
...
flowchart LR
    ...
    T7["T7: eval-harness Verified gate"]
