...
- `supabase/migrations/20260425000001_rls_domain_model.sql` — Per-table RLS enabling + bifurcated policies
...
-- pliego: same pattern; deduplication by file_hash enforced at app layer
...
```sql
-- analisis: SELECT/INSERT/UPDATE only if user belongs to analisis.empresa_id
...
-- prompt_cache: scoped by (pliego_id, empresa_id) — empresa_id directly available
...
```sql
-- empresa: SELECT only if user is a member of that empresa
...
-- empresa_member: SELECT own membership rows, or rows of empresas where user is owner
...
These make DELETE always fail for authenticated users on `pliego` and `anexo_proceso`. Service processes use the service role only for soft-delete (`UPDATE deleted_at`). `Proceso` does not need hard-delete prevention — it has no `deleted_at` column at all and deletion is simply not part of the model (RN-004).

### Design Rationale (Defense in Depth)

RLS lives in a separate migration from DDL (T3) so that: (a) table structure and access control can be reviewed independently; (b) RLS can be modified without recreating tables; (c) policy tests (TC-006, TC-008, TC-009) can be run against T3 alone first to verify table correctness. The bifurcation between public and empresa-scoped tables is the security model's cornerstone — public records cannot leak competitive intelligence because no empresa-private data lives in them.

## Dependencies

Requires T3 — tables must exist before RLS can be enabled on them.

## Done When

- [ ] Migration file exists at `supabase/migrations/20260425000001_rls_domain_model.sql`
...
- [ ] TC-017 passes: `anexo_proceso` SELECT returns the same row to users of two different empresas (public-read parity)
